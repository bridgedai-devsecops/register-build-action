...
exports.safeLogMessage = safeLogMessage;
const SENSITIVE_KEY_RE = /(token|secret|password|authorization|cookie|bearer|apikey|api_key|private[_-]?key|ssh[_-]?key|ghs_|ghp_|github[_-]?token|oidc)/i;
...
function redactStringValue(value) {
    ...
    if (value.startsWith('brdg_'))
        return '[REDACTED_API_KEY]';
    out = out.replace(API_KEY_PATTERN, '[REDACTED_API_KEY]');
    out = out.replace(JWT_PATTERN, '[REDACTED_JWT]');
    if (out.length > 2000)
        return `[REDACTED_LARGE_STRING_LEN_${out.length}]`;
    return out;
}
function redactUnknown(input, depth = 0) {
    if (depth > 32)
    ...
    if (Array.isArray(input))
        return input.map((x) => redactUnknown(x, depth + 1));
...
function safeLogMessage(message, apiKey) {
